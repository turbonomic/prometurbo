...
func (k *k8sTopologyHelper) handleNvidiaGPU(entity *data.DIFEntity) {
	...
	// Derive gpu and gpuMem capacities
	gpuMemSizeGb := entity.GetAttribute("gpuMemSizeGb")
	for _, gpuMetric := range entity.Metrics["gpu"] {
		gpuAverage := *gpuMetric.Average / 100
		gpuMetric.Average = &gpuAverage
		gpuCapacity := 1.0 // 1 GPU
		gpuMetric.Capacity = &gpuCapacity
	}
	if gpuMemCapacity, err := strconv.ParseFloat(gpuMemSizeGb, 64); err == nil {
		for _, gpuMemMetric := range entity.Metrics["gpuMem"] {
			gpuMemMetric.Capacity = &gpuMemCapacity
			gpuAverage := *gpuMemMetric.Average / 100 * gpuMemCapacity
			gpuMemMetric.Average = &gpuAverage
		}
	} else {
		glog.Infof("GPU memory capacity cannot be extracted from this GPU %s of model %s assigned to pod %s.",
			entity.UID, gpuModel, podName)
	}
