...
cp -r ${SRC_DIR}/prometurbo-operator/deploy/*.y* operator/
cp -r ${SRC_DIR}/prometurbo-operator/deploy/crds/* operator/

# copy yaml files
echo "===> Copy yaml files"
cp -r ${SRC_DIR}/prometurbo_yamls/* yamls/

# Insert current version
echo "===> Updating Turbo version in yaml files"
sed -i.bak "s|version: 1.0.0|version: ${VERSION}|" helm_chart/Chart.yaml 
find ./ -type f -name '*.y*' -exec sed -i.bak -e "s|<PROMETURBO_IMAGE_TAG>|${VERSION}|g; s|<TURBODIF_IMAGE_TAG>|${VERSION}|g" {} +
